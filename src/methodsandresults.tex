...
Magic instructions denotes \dvtcmdcodeinline{nop}-type instructions, meaning instructions that would have no effect if run on the target architecture (such as \dvtcmdcodeinline{xchg ebx, ebx} on x86-compatible architectures).
...
The simulation systems are, like the host system, set up to run Fedora~$19$ Linux and use the Mesa llvmpipe driver~\dvtcmdciteref{web:mesa:2015} for software rasterization.
...
These benchmark instances are performed with smaller and larger input data, tuned to yield approximately half and double frame time, respectively, assuming that the benchmarks really stress the bottlenecks they was designed for.\todo{Elaborate?}\
...
It is so named because the program calculates the Julia fractal~\dvtcmdciteref{web:tsiombikas:2014}.
