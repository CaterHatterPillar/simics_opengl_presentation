...
When executed on the simulated hardware in a virtual platform, it invokes a callback-method in the simulation host~\masccitebib[p.~32]{publications:leupers:2010}.
...
The simulation systems are, like the host system, set up to run Fedora~$19$ Linux and use the Mesa llvmpipe driver~\mascciteref{web:mesa:2015} for software rasterization.
...
The developed benchmark suite is open source and available online~\masccitebib{web:intel:2014}.
...
It is so named because the program calculates the Julia fractal~\mascciteref{web:tsiombikas:2014}.
