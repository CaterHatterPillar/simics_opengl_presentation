...
As such, it is responsible for transferring a serialized command stream between target software and the simulation host.
...
Such a scenario would be a debugging breakpoin or, in our case, share data between target and host systems.
There are a number of ways to accomplish this, such as networking means or specially devised kernel drivers, but few are as instant as the "magic instruction".

Magic instructions denotes \dvtcmdcodeinline{nop}-type instructions, meaning an instruction that would have no effect if run on the target architecture (such as \dvtcmdcodeinline{xchg ebx, ebx} on x86-compatible architectures).
...
Because of the inherent performance demands brought on by real-time graphics, magic instructions is a suitable candidate for the exchange of information between target and host systems.
...
Having sucessfully escaped simulation, we may assume the presence of a target virtual address in the agreed-upon CPU register.
...
Accordingly, this address need to be translated in order to retrieve the package contents that the memory address points to.
...
%% Advantages of magic instructions include often negligible invocation cost, as the context switch is often instant from the perspective of the target system~\dvtcmdcitebib[p.~131]{journals:rechistov:2013}.
