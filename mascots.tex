%% mascots.tex

% http://www.ieee.org/publications_standards/publications/authors/author_guide_interactive.pdf recommend the following sections:
% Abstract; Introduction; Previous Research; Problem Formulation; Model or Methods and Results; Conclusion; References; Acknowledgements

% Replace old tags with new ones when the majority of contents have
% been imported to the new document.
% perl -pi -w -e 's/cha:introduction/sec:introduction/g;' src/*.tex
% perl -pi -w -e 's/cha:backgroundandrelatedwork/sec:previousresearch/g;' src/*.tex
% perl -pi -w -e 's/cha:aimsandobjectives/sec:problemformulation/g;' src/*.tex

% cha:proposedsolutionandimplementation -> sec:methodsandresults
% cha:experimentalmethodology -> sec:experimentalmethodology
% cha:results -> sec:results
% cha:conclusion -> sec:conclusion

% Thesis chapters yet to import contents from:
% * Simics (under previous research or introduction?)
% * Threats to validity (the first section probably, but the second one can hopefully be skipped since we won't be (in the future) presenting the texture benchmark results.
% * Discussion
% * Conclusion
% * Future work

\documentclass[conference]{IEEEtran}
% If IEEE MASCOTS is part of the IEEE Computer Society- range of
% conferences, be sure to add the compsoc option to the document class.
% If IEEEtran.cls has not been installed into the LaTeX system files,
% manually specify the path to it like:
%\documentclass[conference]{../sty/IEEEtran}

\usepackage{soul} % Used to temporarily highlight areas of text during writing.
\usepackage{xcolor} % colorbox for the inline code command.
\usepackage{hyperref} % Hyperlinks and urls.
\usepackage{listings} % code boxes.
\usepackage{amsfonts} % \circledR.
\usepackage{graphicx} % Displaying pdf graphics.
\usepackage{multirow} % Used for specialized tables (multirow).
\usepackage{rotating} % Used to rotate figures sideways.
\usepackage[mediumspace,mediumqspace,squaren,binary]{SIunits} % \milli\second

% correct bad hyphenation here
\hyphenation{op-tical net-works semi-conduc-tor}

% These should probably be merged into the same bibliography list in
% the end, but it might be good to have them seperately now so that
% it's easier to filter out possibly unreliable sources.  When most
% contents have been imported from the thesis, replace these with
% suitable commands.
\newcommand{\dvtcmdcitebib}[2][]{\cite{#2}} % TODO: \masccitebib
\newcommand{\dvtcmdciteref}[2][]{\cite{#2}} % TODO: \mascciteref
%\newcommand{\dvtcmdcitefur}[2][]{\citefur{#2}}
\newcommand{\dvtcmdrefname}[1]{\nameref{#1}}

% Formatting inline code command:
\newcommand{\dvtcmdcodeinline}[1]{\colorbox{black!5}{\lstinline[basicstyle=\ttfamily\color{black}]{#1}}}
% Command to read, and ouput the first line of a file:
\newread\file
\newcommand{\dvtcmdfirstline}[1]{
  \openin\file=#1
  \read\file to \keyval
  \keyval % Return keyval.
  \closein\file
}

\begin{document}

% Placeholder title:
\title{%
  Accelerating Graphics in\\
  the Simics Full-system Simulator}

\author{\IEEEauthorblockN{Eric Nilsson}
\IEEEauthorblockA{Intel Corporation\\
Email: \href{mailto:eric.nilsson@intel.com}{eric.nilsson@intel.com}}}

\maketitle

\input{src/abstract}
\input{src/introduction}
\input{src/previousresearch}
\input{src/problemformulation}
\input{src/methodsandresults}
\input{src/conclusion}

% For peer review papers, you can put extra information on the cover
% page as needed:
% \ifCLASSOPTIONpeerreview
% \begin{center} \bfseries EDICS Category: 3-BBND \end{center}
% \fi
%
% For peerreview papers, this IEEEtran command inserts a page break and
% creates the second title. It will be ignored for other modes.
\IEEEpeerreviewmaketitle

\section*{Acknowledgment}
\ldots

\bibliographystyle{IEEEtran}
\bibliography{mascots}

\end{document}

% Notes:
% no keywords
% conference papers do not normally have an appendix
