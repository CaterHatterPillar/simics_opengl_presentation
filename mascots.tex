...
%   section. Currently commented out under Previous Research.
% * Consider removing the sections on Simics Deterministic Execution,
%   Checkpointing and Reverse Execution. Alternatively, take the relevant
%   contents and just stick them under the overhead Simics section without
%   seperate rubrics.
...
% * The 'input variations' tables under section Benchmarks could probably be
%   made considerably smaller now that the Phong benchmark is not involed
%   anymore. Do they have to be seperate tables?
% * Include the platfor profiling bits from Threats to Validity when describing
%   how the time samples were collected.
...
%   bring up QEMU.
% * (Consider) Don't call the benchmarks 'Chess' and 'Julia'. Instead refer
%   to them as the latency benchmark and the performance benchmark, or similar.
...
% * Import the platform profiling section. It must be made abundently clear that
%   all values presented in this material adhere to the profiling overhead
...
% * Threats to validity (the first section probably, but the second one can
%   hopefully be skipped since we won't be (in the future) presenting the
%   texture benchmark results.
% * Discussion
% * Future work
